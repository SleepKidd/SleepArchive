...
На Windows откройте обычную `cmd.exe` в корне репозитория и выполните:
...
5. сгенерирует ICO;
6. запустит тесты и остановится при ошибке;
7. очистит старые `build/` и `dist/`;
8. соберёт onefile-приложение без консольного окна;
9. проверит наличие результата.
